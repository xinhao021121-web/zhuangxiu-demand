...
/* ---------- 新增字段 ---------- */
const NEW_ROWS = [
  ["base_other", "基础信息", "其他", "其他补充", "长文本", "文本", "选填", "个性化需求或特殊说明", "本次重构", "助手建议会写回这里"],
  // 基础信息
  // 设备与系统
  ["dev_heat", "设备与系统", "空调与采暖", "取暖方式", "多选", "地暖 / 暖气片墙暖 / 只靠空调 / 不确定", "选填", "地面抬高与地板材质", "本次重构", "原「地暖」「墙暖」两项合并"],
  // 其他卧室（可增删实例）
  ["room_type", "其他卧室", "房间定义", "这个房间主要用来做什么", "单选", "儿童房 / 长辈房 / 客房 / 书房 / 电竞房 / 多功能房 / 其他", "实例必选", "决定该房间渲染哪些字段", "本次重构", "选完类型后只显示相关字段"],
  ...
  // 卫生间（可增删实例）
  ["wc_type", "卫生间", "房间定义", "这间卫生间", "单选", "主卫 / 客卫 / 次卫 / 其他", "实例必选", "决定显示名称与差异化建议", "本次重构", "默认主卫、客卫两间"],
  // 厨房
  // 收纳与家政
...
const SUGGEST = new Set([
  "base_house_state", "base_area", "live_members", "live_elder", "live_pet",
  "budget_total", "date_movein", "kt_freq", "wc_bath",
]);

/* ---------- 空间实例作用域 ---------- */
/* 实例必选：添加实例时必须选择，否则该实例无法渲染字段 */
const INSTANCE_REQUIRED = new Set(["room_type", "wc_type"]);
...
function fromV1(r) {
  ...
  return [
    ...
    INSTANCE_REQUIRED.has(id) ? "实例必选" : (SUGGEST.has(id) ? "推荐填写" : "选填"),
    SCOPE[id] || "固定",
    APPLIES[id] || "全部",
    rule || "-",
    source,
